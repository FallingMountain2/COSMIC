...
function initSpeed() {
	...
	if (game.rocket.upgrade[1] > 50) {
		upgradePow = upgradePow.plus(Decimal.mul(0.0003, game.rocket.upgrade[1]));
	}
...
function initExchangeRate() {
	...
	if (game.rocket.upgrade[0] > 100) exchangeUpgradePow = new Decimal(1.3).plus(Decimal.mul(0.001, game.rocket.upgrade[0]));
...
function initMaxHeight() {
	...
	if (game.rocket.upgrade[3] >= 50) maxHeightPow = new Decimal(1.65);
